...
async def new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Welcome each new member, then show rules
...
async def ban_inappropriate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Guard in case message is None for some updates (shouldn’t happen due to filter, but safe)
...
# ---------- Run PTB in its own event loop thread (safe with Flask/Gunicorn) ----------
